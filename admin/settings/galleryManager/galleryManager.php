			...
			if(isset($_GET['function']) && 'gallery'==$_GET['function']) { ?>
				...
				foreach($images as $image)
				{
					...
					if(preg_match('#^(.+?)(_t)?\.(jpg|gif|png)#i', $image)){
						...
						echo '<div class="galleryDisplay"><img src="'.$newImageURL.'" /><input type="button" class="transition" value="Ok!" onclick="parent.SGMAddImage(\''.$newImageURL.'\');parent.hideMask();self.location.href=\'clearGM.php\'"/></div>' ;
					}
				}
			}
